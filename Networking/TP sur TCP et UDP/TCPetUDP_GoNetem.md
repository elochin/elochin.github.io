...
Pour cela, vous allez utiliser la topologie de test suivante (téléchargez fichier [*TP_UDP_TCP.gnet*](https://e-campus.enac.fr/moodle/mod/resource/view.php?id=119385) pour cela) et configurez correctement les routes des deux PC afin qu'ils puissent mutuellement s'atteindre :
```bash
PC1 ------------------------------ R1 ------------------------------ PC2
...
Téléchargez l'application **tsock** dans votre terminal maître en suivant ce [*lien*](https://e-campus.enac.fr/moodle/pluginfile.php/196816/mod_resource/content/1/tsock). Puis copiez ce programme sur PC1 et PC2. Pour cela depuis la console (en supposant que vous avez téléchargé **tsock**  dans le répertoire Téléchargements par défaut) :
```
...
1. Lancez maintenant un serveur UDP sur PC1 en ajoutant l'option `-u` comme suit `tsock -s 10000 -u`. Faire de même côté client avec PC1 : `tsock -c 10.1.1.1 10000 -u`. Quelles différences observez-vous avec TCP ?
