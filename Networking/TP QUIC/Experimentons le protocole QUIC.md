...
2. sur PC0 lancez `iperf3 -c 10.1.1.1 -t 1000`. Attendez que le débit se stabilise à 10Mb/s
3. sur PC2 `iperf3 -c 10.1.1.2 -t 1000 -u -b 2M`. Quel débit est obtenu par les deux flots ?
...
Pour cela dans le shell de la console tapez `copy picoquicdemo SRC:/root` puis `copy picoquicdemo SRC:/root`.
...
- Sur la machine qui fera office de serveur :
  ...
  - lancer le serveur avec la commande suivante : `./picoquicdemo -b logs/ -w www/`

- Sur la machine cliente :
  - créer un répertoire `logs/`
  - lancer le client avec : `./picoquicdemo -b logs/ 10.0.0.2 4443` ou `10.0.0.2` est l'adresse IP du serveur. On obtient alors tout un tas de statistiques avec notamment la vitesse du transfert donné par la ligne qui commence par `Received` par exemple chez moi : `Received 22206148 bytes in 0.221566 seconds, 801.789011 Mbps.`
...
1. Prenez le temps d'analyser les options disponibles et retournées par les stats qlogs ;
2. Générez des transferts de 2, 5 et 10MB avec quic-go et picoquic, effectuez une série permettant d'avoir une moyenne/écart-type des temps de transferts (*download time*) représentatifs des expérimentations ;
